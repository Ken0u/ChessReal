...
public class Rook extends Piece {
    ...
    public Rook(int x, int y, boolean isWhite) {
        super(isWhite);
        this.isRook = true;
        rookView = new ImageView();
        this.x = x;
        this.y = y;
        if (isWhite) {
            rookView.setImage(new Image(path + "white_rook.png"));
        } else {
            rookView.setImage(new Image(path + "black_rook.png"));
        }
    }
    ...
    public boolean isValidMove(int newY, int newX, Board board) {
        if (board.getPieceAt(newY, newX) != null ) {
            if (board.getPieceAt(newY, newX).isWhite() != this.isWhite()){
                return (newX == x || newY == y) && !isPathBlocked(newY, newX, board);
            }else{
                return false;
            }
        }else{
